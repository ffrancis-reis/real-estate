...
contract Ownable {
    ...
    modifier onlyOwner() {
        require(
            _owner == msg.sender,
            "Ownable Caller is not the conrtact owner"
        );
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        ...
        require(newOwner != address(0x0), "Address is not valid");
...
contract Pausable is Ownable {
    ...
    event Paused(address indexed _address);
    event Unpaused(address indexed _address);
    ...
    function setUnpause() public onlyOwner paused {
        _paused = false;
...
contract ERC721 is Pausable, ERC165 {
    ...
    function approve(address to, uint256 tokenId) public {
        ...
        require(to != ownerOf(tokenId), "Address is the owner of the token id");

        require(
            msg.sender == ownerOf(tokenId) ||
                isApprovedForAll(ownerOf(tokenId), msg.sender),
            "ERC721 Caller is not the conrtact owner"
        );
    ...
    function _mint(address to, uint256 tokenId) internal {
        ...
        require(
            _tokenOwner[tokenId] == address(0x0),
    ...
    function _transferFrom(
        ...
        require(
            ...
            "Origin address is not the token owner"
        );
        require(to != address(0x0), "Destination address is not valid");
